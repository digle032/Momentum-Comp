...
const navItems: { id: AppPage; label: string; icon: React.ReactNode; description: string }[] = [
  { id: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard size={18} />, description: 'Overview & activity' },
  { id: 'athletes',  label: 'Athletes',  icon: <Users size={18} />,           description: 'Manage profiles' },
  { id: 'teams',     label: 'Teams',     icon: <Shield size={18} />,          description: 'Groups & rosters' },
  { id: 'calendar',  label: 'Calendar',  icon: <Calendar size={18} />,        description: 'Schedule sessions' },
  { id: 'studio',    label: 'Studio',    icon: <Camera size={18} />,          description: 'Create compilations' },
]
...
export const MainSidebar: React.FC<MainSidebarProps> = ({ activePage, onNavigate }) => (
  <aside className="relative z-10 w-[240px] h-full flex flex-col border-r border-border bg-card/80 backdrop-blur-panel shrink-0">
    ...
    <div className="px-4 py-4 border-t border-border shrink-0">
      <p className="text-[11px] text-muted-foreground text-center">Momentum v1.0 — Coach Edition</p>
    </div>
  </aside>
)
